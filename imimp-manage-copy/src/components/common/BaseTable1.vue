  ...
  <div class="table">
    ...
    <div class="container">
      ...
      <el-table :data="tableData" :default-expand-all="false" class="table" ref="multipleTable">
        ...
        <!--el-table-column label="自定义菜单" align="center">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handlePZ(scope.row.id,scope.row.industry_id)">配置
            </el-button>
          </template>
        </el-table-column>
        <el-table-column label="功能权限" align="center">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handleSZ(scope.$index, scope.row.id, scope.row.project_name)">设置
            </el-button>
          </template>
        </el-table-column-->
  ...
  export default {
    ...
    // 获取 easy-mock 的模拟数据
      ...
      handleSZ(index, rowId, rowProName) {
        console.log(index, rowId, rowProName);
        window.open("http://"+rowProName+"/consoleLoginPage", "window","location=no");

      },

  //活动管理
      handleAct(rowProName){
        window.open("http://"+rowProName+"/pcSc","_blank");
      },
      //确定新增项目
      sureSave(formName) {
